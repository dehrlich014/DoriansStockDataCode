...
####The individual data.frames of probabilities show 121 probabilities that (Delta_tDayPost > dt | Delta_rDayPrior > dr)
...
####***What's needed, however, is a sample space, S, that has computed each of the rDayPriors and tDayPosts
	...
	for(t in 1:length(posts)){
		...
		for(j in 0:10){
			...
			for(i in 0:10){
				...
				#Is that A is an empty set since R returns NA when the size of an event in a sample space is 0
				...
				#We want pCount = 121, if we are taking i,j \in {0,1,2,...,10} and each
				#Since the only "false errors" were NA's when P(A) = 0, we now should only have NA's if there is a true error
		...
		#After we store currTable in the r,t position in the tessaract we increment our pCountIndex by 1
